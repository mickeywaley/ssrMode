...
class Server():
    ...
    def listen_client(self):
        while 1:
            ...
            print(address+'进来了')
            print('shell启动正常')
            print('阻塞没优化，详情参考这个：https://www.jianshu.com/p/8e582146bd4c')
    ...
    def send_data(self, data, address):
        import struct,subprocess,os,signal
        from urllib import unquote
        try:
         username = unquote(self.users[address])
         process = subprocess.Popen(data,stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True,preexec_fn=os.setsid)
        except:
            username = '匿名用户'
        if data:
